...
function Initialize()
	...
	MeasureGeneratorWave(peak,tonumber(SKIN:GetVariable('Preview')),'prominence')
...
function Update()
	...
	for i = 1, peak do
		drawWave(	audioMeasure[i==1 and peak or (i-1)]:GetValue(),
					...
					audioMeasure[i==1 and (2*peak) or (i+peak-1)]:GetValue(),
					audioMeasure[i+peak]:GetValue(),
					audioMeasure[i<=peak-1 and (i+peak+1) or i==peak and (1+peak)]:GetValue(),
					audioMeasure[i==peak-1 and (1+peak) or i==peak and (2+peak) or (i+peak+2)]:GetValue(),
					i
				)
	end
end

function drawWave(audio1,audio2,audio3,audio4,audio5,audio6,audio7,audio8,peakcalc)
	for i = 0,totalMu-1 do 
		...
		for j = 1, maxWave do
			local change = Y1+(Y2-Y1)*(j-1)/(maxWave-1)
